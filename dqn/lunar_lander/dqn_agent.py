...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        # the act() method is responsible for ensuring the local model is in train mode so we don't need to do that here
        
        # get the actions predicted by the target model (result is tensor of [batch_size, 1])
        # detach it from the computational graph so that gradients won't be backpropagated through it (we only want to
        # do that on the local network)
        target_actions = self.qnetwork_target(next_states).detach()
        
        # Find the max Q value for each state; (column 0 of the named tuple from max(), then convert to a column tensor [b, 1]
        ...
        # compute the loss & weight gradient and apply to the target model
